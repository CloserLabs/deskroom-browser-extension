...
function IndexPopup() {
  ...
  return (
    <Flex className="w-60 h-64 px-4 py-2">
      <Flex className="container" direction="column">
        ...
        <Flex className="py-2" gap="3">
          <Box className="my-2 flex flex-col">
            ...
            <input type="text" id="org" value={orgs?.currentOrg?.key || ""} placeholder="소속" disabled />
          </Box>
          <Separator size="4" />
          <Box className="my-2 flex flex-col">
            <label htmlFor="account" className="text-gray-400">로그인 된 계정</label>
      ...
      <Box className="text-center py-4 flex align-center justify-center cursor-pointer" onClick={() => {
